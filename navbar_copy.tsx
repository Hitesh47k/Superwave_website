...
export default function Navbar() {
  ...
  return (
    <nav className={`sticky top-0 z-50 transition-all duration-300 ${
      isScrolled 
        ? 'bg-[#0F172A] py-3 shadow-lg dark:glass dark:bg-slate-900/70 dark:shadow-md' 
        : 'bg-[#0F172A] py-5 dark:bg-slate-950 dark:shadow-none dark:border-none'
    }`}>
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex justify-between items-center w-full">
              ...
              <Link
                ...
                className={`px-2 xl:px-3 py-2 rounded-md text-[14px] xl:text-[16px] font-semibold tracking-wide transition-colors flex items-center space-x-1 nav-link-underline ${
                  location.pathname === link.path 
                    ? 'text-brand-400 after:scale-x-100 after:origin-bottom-left' 
                    : 'text-white dark:text-slate-300 hover:text-brand-400 dark:hover:text-brand-400'
                }`}
              >
                <span>{link.name}</span>
                ...
                <AnimatePresence>
                  {activeDropdown === link.name && (
                    <motion.div
                      ...
                      className={`absolute top-full right-0 lg:left-0 mt-2 ${link.isMegaMenu ? 'w-80' : 'w-56'} glass-card bg-[#0F172A]/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl p-2 shadow-xl border border-slate-800 dark:border-white/10 z-50`}
                    >
                      {link.dropdown.map((item) => (
                        <Link
                          ...
                          className={`flex items-start px-4 py-3 rounded-xl transition-all duration-300 group/item ${
                            link.isMegaMenu 
                              ? 'hover:bg-slate-800 dark:hover:bg-brand-900/20' 
                              : 'hover:bg-slate-800 dark:hover:bg-white/5'
                          }`}
                        >
                          {item.icon && (
                          ...
                          <div>
                            <div className={`text-sm font-semibold text-white dark:text-slate-200 group-hover/item:text-brand-400 dark:group-hover/item:text-brand-400 transition-colors ${!item.desc && 'py-1'}`}>
                              {item.name}
                            </div>
                            {item.desc && (
        ...
        <div className="hidden lg:block pl-4">
          <Link 
            to="/contact" 
            className="bg-brand-600 dark:bg-brand-600 text-white dark:text-white px-6 py-2.5 text-[14px] xl:text-[16px] font-semibold rounded-lg hover:bg-brand-500 dark:hover:bg-brand-500 hover:text-white dark:hover:text-white shadow-lg shrink-0 whitespace-nowrap btn-premium hover:scale-[1.02]"
          >
            Contact Us
          </Link>
        </div>
        ...
        {/* Mobile Menu Toggle */}
        <button 
          className="lg:hidden p-2 text-white dark:text-slate-300"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden bg-[#0F172A] dark:glass dark:bg-slate-950/95 border-t border-slate-800 dark:border-slate-800 overflow-hidden"
          >
                  ...
                  {link.dropdown ? (
                    <div className="space-y-1">
                      <button 
                        ...
                        className="w-full flex items-center justify-between px-4 py-3 rounded-md text-[15px] font-semibold text-white dark:text-white hover:bg-slate-800 dark:hover:bg-slate-900/50 transition-colors"
                      >
                        <span>{link.name}</span>
                      ...
                      <AnimatePresence>
                        {expandedMobileMenus[link.name] && (
                          ...
                          >
                            <div className="pl-4 space-y-1 border-l-2 border-slate-800 dark:border-slate-800 ml-4 py-2">
                              {link.dropdown.map((item) => (
                                <Link
                                  key={item.name}
                                  to={item.path}
                                  onClick={() => setMobileMenuOpen(false)}
                                  className="flex items-center px-4 py-2 rounded-md text-[15px] font-semibold text-slate-300 dark:text-slate-400 hover:bg-slate-800 dark:hover:bg-slate-900/50 hover:text-brand-400 dark:hover:text-brand-400"
                                >
                    ...
                    <Link
                      to={link.path}
                      onClick={() => setMobileMenuOpen(false)}
                      className="block px-4 py-3 rounded-md text-base font-semibold text-white dark:text-slate-200 hover:bg-slate-800 dark:hover:bg-slate-800"
                    >
                      {link.name}
                    </Link>
                  )}
